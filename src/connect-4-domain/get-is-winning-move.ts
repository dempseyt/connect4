...
function getUpToThreeDisksUnderneathTargetCell(
  ...
  switch (depthToCheck) {
    ...
    case 2:
      return [board[targetRow - 1][targetColumn], board[targetRow - 2][targetColumn]]
    default:
      return [
...
function isVerticalWin(board: Board, playerMove: PlayerMove): { isWin: boolean } {
  ...
  const activePlayer = playerMove.player
  const upToThreeDisksUnderneathTargetCell = getUpToThreeDisksUnderneathTargetCell(
    board,
    playerMove,
  )
...
function getIsWinningMove(board: Board, playerMove: PlayerMove): { isWin: boolean } {
  return {
    isWin:
